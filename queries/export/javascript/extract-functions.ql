...
from Function f
select f.toString() + "@" + f.getFile().getRelativePath() + ":" +
    f.getLocation().getStartLine().toString() as codeql_id, f.getName() as name,
  f.getFile().getRelativePath() as file, f.getLocation().getStartLine() as line,
  f.getNumParameter() as num_params, f.toString() as signature
